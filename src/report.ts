...
import { BatchComparisonResult, ComparisonResult } from './types';
import { StyleDifference, generateFixPrompt } from './style-diff';
import { markdownToHtml } from './markdown';
import { generateAiFixPrompt } from './gemini';
...
const formatNumber = (value?: number) => (isFiniteNumber(value) ? value.toLocaleString() : '–');
const formatMs = (value?: number) => (isFiniteNumber(value) ? `${value.toFixed(0)} ms` : '–');

const buildSingleJson = (result: ComparisonResult) => ({
  runId: result.runId,
  timestamps: {
  ...
  base: {
    ...
    screenshot: path.basename(result.base.screenshotPath),
    timing: result.base.timing,
  },
  target: {
    url: result.target.url,
    label: result.target.label,
    screenshot: path.basename(result.target.screenshotPath),
  ...
  diff: {
    ...
    diffImage: path.basename(result.diff.diffPath),
  },
  styleDiff: result.styleDiff,
  fixPrompt: undefined as string | undefined,
});

const buildBatchJson = (batch: BatchComparisonResult) => ({
  ...
  pages: batch.pages.map((p) => ({
    ...
    base: {
      ...
      screenshot: path.basename(p.base.screenshotPath),
    },
    target: {
      url: p.target.url,
      timing: p.target.timing,
      screenshot: path.basename(p.target.screenshotPath),
    },
    diff: {
      diffPixels: p.diff.diffPixels,
      totalPixels: p.diff.totalPixels,
      similarity: p.diff.similarity,
      diffImage: path.basename(p.diff.diffPath),
    },
...
type SinglePayload = ReturnType<typeof buildSingleJson>;
type BatchPayload = ReturnType<typeof buildBatchJson> & {
  outputDir?: string;
  pages: (ReturnType<typeof buildBatchJson>['pages'][number] & { outputDir?: string })[];
};

const renderStyleDifferences = (differences: StyleDifference[]) => {
  if (!differences || differences.length === 0) return '';

  const sections: Record<string, StyleDifference[]> = {};
  ...
  const renderSectionRows = (sectionName: string, entries: StyleDifference[]) => {
    const groupedByCategory: Record<string, StyleDifference[]> = {};
...
export const buildSingleHtml = (payload: SinglePayload) => {
  ...
  <div class="page">
    ...
    <div class="card" style="margin-top: 14px;">
      ...
      <div style="background: #fff; padding: 16px; border-radius: 8px; font-size: 14px; color: #334155; border: 1px solid #e2e8f0; line-height: 1.6;">
        ${markdownToHtml(payload.fixPrompt)}
      </div>
    </div>
    ` : ''}
...
export const buildBatchHtml = (payload: BatchPayload) => {
  ...
  const pageCards = payload.pages
    .map((page, idx) => {
      ...
      const pageDir = page.outputDir || (page.runId ? path.join(batchRoot, page.runId) : batchRoot);

      let prefix = '';
      try {
        const relativeDir = toPosixPath(path.relative(batchRoot, pageDir));
...
export const writeReport = async (result: ComparisonResult): Promise<string> => {
  ...
  if (result.styleDiff && result.styleDiff.differences.length > 0) {
    ...
    try {
      const prompt = await generateAiFixPrompt(
        result.base.screenshotPath,
        result.target.screenshotPath,
        result.diff.diffPath,
        result.styleDiff.differences
      );
      await fs.writeFile(path.join(result.outputDir, 'fix-prompt.md'), prompt, 'utf-8');
      payload.fixPrompt = prompt;
    } catch (err) {
      console.error('Failed to generate AI fix prompt, falling back to static:', err);
      const staticPrompt = generateFixPrompt(result.styleDiff.differences);
      await fs.writeFile(path.join(result.outputDir, 'fix-prompt.md'), staticPrompt, 'utf-8');
      payload.fixPrompt = staticPrompt;
    }
  }

  await fs.writeFile(reportPath, JSON.stringify(payload, null, 2), 'utf-8');
  await fs.writeFile(path.join(result.outputDir, 'report.html'), buildSingleHtml(payload), 'utf-8');

  return reportPath;
};

export const writeBatchReport = async (batch: BatchComparisonResult): Promise<string> => {
  const reportPath = path.join(batch.outputDir, 'batch-report.json');
  const payload = buildBatchJson(batch);

  await fs.writeFile(reportPath, JSON.stringify(payload, null, 2), 'utf-8');
  await fs.writeFile(path.join(batch.outputDir, 'batch-report.html'), buildBatchHtml(payload), 'utf-8');
  return reportPath;
};
