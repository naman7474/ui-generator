...
async function main() {
    ...
    try {
        ...
        const { data: data1, error: error1 } = await supabase.from('runs').select('id').limit(1);
        if (error1) console.error('Error 1:', error1);
        else console.log('Success 1:', data1);

        console.log('Attempt 2: Select * without order...');
        const { data: data2, error: error2 } = await supabase.from('runs').select('*').limit(1);
        ...
        const { data: data4, error: error4 } = await supabase.from('runs').insert(dummyRun).select();
        if (error4) console.error('Error 4:', error4);
        else console.log('Success 4:', data4);
