...
export interface ComputedStyleData {
  ...
  className: string;
  selector: string;
  text?: string; // Captured text content for context
  rect: { x: number; y: number; width: number; height: number };
  styles: Record<string, string>;
  sectionTrail: string[];
  children: ComputedStyleData[];
}

export type StyleCategory = 'Typography' | 'Spacing' | 'Layout' | 'Color' | 'Effects' | 'Misc';
...
export interface StyleDifference {
  ...
  baseValue: string;
  targetValue: string;
  diff: string; // e.g., "+4px", "changed"
  category: StyleCategory;
  section?: string;
  sectionPath?: string;
  sectionSource?: 'base' | 'target';
  sectionBase?: string;
  sectionTarget?: string;
  sectionPathBase?: string;
  sectionPathTarget?: string;
}
...
const getCategory = (property: string): StyleCategory => {
  if (property.includes('font') || property.includes('text') || property.includes('line-height')) return 'Typography';
  if (property.includes('padding') || property.includes('margin') || property === 'gap') return 'Spacing';
  if (property.includes('width') || property.includes('height') || property.includes('display') || property.includes('flex') || property.includes('position') || ['top', 'right', 'bottom', 'left', 'z-index'].includes(property)) return 'Layout';
  if (property.includes('color') || property.includes('background')) return 'Color';
  if (property.includes('shadow') || property.includes('opacity') || property.includes('visibility')) return 'Effects';
  return 'Misc';
};

export const extractComputedStyles = async (page: Page): Promise<ComputedStyleData[]> => {
  return page.evaluate((relevantStyles) => {
    ...
    const inferAriaLabel = (element: Element): string | null => {
    ...
    const qualifiesAsSection = (element: Element): boolean => {
      const tag = element.tagName.toLowerCase();
      if (SECTION_TAGS.includes(tag)) return true;

      const role = element.getAttribute('role')?.toLowerCase();
      if (role && SECTION_ROLES.includes(role)) return true;

      if (SECTION_DATA_ATTRS.some(attr => element.hasAttribute(attr))) return true;
    ...
    const describeSection = (element: Element): string | null => {
      if (!qualifiesAsSection(element)) return null;
    ...
    const traverse = (element: Element, indexInParent: number, sectionTrail: string[]): ComputedStyleData | null => {
      ...
      const children: ComputedStyleData[] = [];
      let childIndex = 0;
...
const flattenStyleData = (data: ComputedStyleData[], parentPath = ''): Map<string, ComputedStyleData> => {
  const map = new Map<string, ComputedStyleData>();
...
export const compareStyleData = (
  base: ComputedStyleData[],
  target: ComputedStyleData[],
  ...
  baseMap.forEach((baseElement, key) => {
    ...
    if (!targetElement) return; // Element removed or moved significantly

    const baseSectionTrail = baseElement.sectionTrail ?? [];
    const targetSectionTrail = targetElement.sectionTrail ?? [];

    const sectionSource: 'base' | 'target' | undefined = targetSectionTrail.length > 0
      ? 'target'
    ...
    for (const prop of RELEVANT_STYLES) {
      ...
      if (baseVal !== targetVal) {
        ...
        if (!isNaN(baseNum) && !isNaN(targetNum)) {
          ...
          if (delta === 0) continue; // Might be different units or string representation, but if numbers match...
...
export const generateFixPrompt = (differences: StyleDifference[]): string => {
  if (!differences || differences.length === 0) {
    return 'No style differences found. The target site is visually identical to the base site.';
  }

  const groupedBySection: Record<string, Record<string, StyleDifference[]>> = {};
  differences.forEach((diff) => {
    ...
    if (!groupedBySection[sectionKey][catKey]) groupedBySection[sectionKey][catKey] = [];
