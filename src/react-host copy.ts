...
const fixIconMisimportsFromReactDom = (code: string): string => {
    ...
    const matches: Match[] = [];
    // Match react-dom or react-dom/client, including esm.sh variants
    const isReactDomSpec = (s: string) => /react-dom(@[^/]+)?(\/client)?(\?.*)?$/.test(s) || /react-dom/.test(s);

    // Regex to capture imports. Note: we want to be careful not to break valid imports.
    // We look for imports from something that looks like react-dom
    ...
    for (const m of matches) {
        ...
        // Known non-icon exports from react-dom
        const allowedReactDom = new Set([
            'createRoot', 'hydrateRoot', 'flushSync', 'unstable_batchedUpdates',
            'useFormStatus', 'useFormState', 'useOptimistic', 'prefetchDNS',
            'preconnect', 'preload', 'preinit'
        ]);

        for (const p of parts) {
            // Treat any PascalCase as icon candidate; react-dom exports are usually camelCase (except specific hooks/types which we might miss, but icons are the main issue)
            // Also explicitly catch known icons if they slip in
            const name = p.replace(/\sas\s+[A-Za-z_$][\w$]*$/, '').trim();

            // If it's a known valid export, keep it
            if (allowedReactDom.has(name)) {
                keep.push(p);
                continue;
            }

            // If it starts with Uppercase (Component/Icon) or is a known icon name, move it
            if (/^[A-Z]/.test(name)) {
                toIcons.push(p);
            } else {
                // If unsure (camelCase but not in allowlist), keep it to be safe, 
                // unless it's clearly an icon name (unlikely for camelCase)
                keep.push(p);
            }
        }
        ...
        // Use a pinned version for consistency if adding new
        const lucideSpec = 'https://esm.sh/lucide-react@0.263.1?dev';
        ...
        if (m2) {
            ...
            // If no existing lucide import, add one. Try to use the same style as existing imports if possible, otherwise default to esm.sh
            out = `import { ${icons.join(', ')} } from '${lucideSpec}';\n` + out;
        }
    }
