...
exports.generateMultiDeviceFixPrompt = exports.generateAiFixPrompt = void 0;
const generative_ai_1 = require("@google/generative-ai");
const config_1 = require("./config");
const server_1 = require("@google/generative-ai/server");
const genAI = config_1.config.googleApiKey ? new generative_ai_1.GoogleGenerativeAI(config_1.config.googleApiKey) : null;
const fileManager = config_1.config.googleApiKey ? new server_1.GoogleAIFileManager(config_1.config.googleApiKey) : null;
...
const generateAiFixPrompt = async (baseImagePath, targetImagePath, diffImagePath, differences) => {
...
Respond with the Markdown plan only.`;
        ...
        for (const modelName of modelPreference) {
            try {
                ...
                console.warn(`Gemini model ${modelName} failed: ${modelError instanceof Error ? modelError.message : String(modelError)}.`);
            }
        }
        if (lastError) {
            throw lastError;
...
exports.generateAiFixPrompt = generateAiFixPrompt;
const generateMultiDeviceFixPrompt = async (artifacts) => {
    if (!genAI || !fileManager) {
        console.warn('GOOGLE_API_KEY not set, skipping AI prompt generation.');
        return 'Google API Key not configured. Please set GOOGLE_API_KEY in .env to generate AI fix prompts.';
    }
...
exports.generateMultiDeviceFixPrompt = generateMultiDeviceFixPrompt;
