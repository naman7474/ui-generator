...
const firstDefined = (...values) => values.find((v) => typeof v === 'string' && v.length > 0);
const supabaseServiceKey = firstDefined(process.env.SUPABASE_SERVICE_ROLE_KEY, process.env.SUPABASE_SERVICE_KEY, process.env.SUPABASE_ADMIN_KEY);
const supabaseAnonKey = firstDefined(process.env.SUPABASE_ANON_KEY, process.env.SUPABASE_PUBLIC_ANON_KEY, process.env.SUPABASE_PUBLIC_KEY, 
...
exports.config = {
    ...
    waitUntil: process.env.NAVIGATION_WAIT_STATE ?? 'networkidle',
    navigationTimeoutMs: numberFromEnv(process.env.NAVIGATION_TIMEOUT_MS, 1200000),
    outputDir: process.env.OUTPUT_DIR ?? path_1.default.join(process.cwd(), 'artifacts'),
